...
export default function AiCameraPractice({
  ...
  const targetText = targetDisplay || requestedTarget?.display || "một trong 6 ký hiệu AI đã train";
  ...
  const handleScan = async () => {
    ...
    try {
      const result = await recognizeGestureFromVideo(videoRef.current, { durationMs: captureMs, fps: 10 });
      ...
      await signatureApi.submitAttempt(
        {
          userStoryId: "US-AI-REAL-PRACTICE",
      ...
      if (correct) {
        ...
        setMessage(
          requestedTarget
            ? `AI nhận diện đúng ${targetText} với độ tin cậy ${Math.round((nextPrediction.confidence ?? 0) * 100)}%.`
            : `AI nhận diện ${aiLabelToDisplay(nextPrediction.label)}. Bài này chưa có class target riêng trong model hiện tại.`
        );
    ...
    <div className="card-pop p-5 text-center">
      ...
      <div className="rounded-[18px] bg-primary/10 border border-primary/20 px-4 py-3 mb-4 text-left">
        <p className="text-xs font-body text-foreground">
          AI service thật đang nhận diện 6 ký hiệu: cà phê, đá, đen, nóng, sữa, trà. FE chỉ gửi frame tạm thời đến AI service để dự đoán; BE chỉ lưu metadata kết quả.
        </p>
      </div>
      {previewMode && (
